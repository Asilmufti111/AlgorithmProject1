...
public class KruskalAlg extends MSTAlgorithm {
		...
		Vertex src; 
                ...
                for(Vertex i:graph.vertices) {    //go thro all vertices
                    ...
                    for(Edge j : src.adjList){//loop thro all source vertices
                        ...
                        edges.add(src.adjList.get(j.weight));}}//store ALL edges in edges<>()}
    ...
    public void makeSet(Vertex[] quickFindDS) { //make all vertices into singleton sets
        ...
        for(Vertex i : quickFindDS){ //loop through all vertices
            ...
            quickFindDS[label] = singleton;} //edit label with singleton set
    ...
    public void displayResultingMST() {
        ...
        for(int i=0; i<MSTresultList.length-1; i++) { //go through all edges
            ...
            v.displayInfo(); System.out.print(" - "); //print info of source
			  
            Vertex u = MSTresultList[i].target; //target vertex of edge i
			  
            u.displayInfo(); System.out.print(" : line name: line x" + (MSTresultList[i].source.label+1) + " ");//print info of target
			  
            Edge e = MSTresultList[i]; //edge i (starting from min edge)
			 
            e.displayInfo(); System.out.println();  //print info of edge 
		   }
	   }
